...
# -*- coding: utf-8 -*-
"""
Script de Prueba: Predicción de Color de Papa usando Random Forest
------------------------------------------------------------------
Objetivo:
    Cargar el modelo Random Forest entrenado y clasificar el color de una 
    papa ingresando la ruta de la imagen.

Uso:
    python predecir_rf_color.py [ruta_de_la_imagen]
...
MODEL_PATH = SCRIPT_DIR / "modelo_rf_colores.pkl"
...
def extraer_caracteristicas_imagen(img_path: Path) -> np.ndarray:
    """
    Extrae las mismas 21 características visuales tabulares del script de entrenamiento:
      - RGB promedio (3)
...
def main():
    print("=" * 70)
    print("  PREDICTOR DE COLOR DE PAPA (RANDOM FOREST)  ")
    print("=" * 70)
    ...
    if not (MODEL_PATH.exists() and SCALER_PATH.exists() and LABEL_ENCODER_PATH.exists()):
        ...
        print(f"Asegúrese de haber ejecutado 'entrenar_rf_colores.py' primero.")
        sys.exit(1)
