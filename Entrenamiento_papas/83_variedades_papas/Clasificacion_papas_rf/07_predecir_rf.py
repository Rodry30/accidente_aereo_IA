...
def clasificar_papa(img_path):
    ...
    if not MODEL_PKL.exists() or not SCALER_PKL.exists() or not LABEL_MAP_PKL.exists():
        ...
        print("Asegúrate de haber ejecutado 03_preprocesar.py y 04_entrenar_rf.py.")
        sys.exit(1)
    ...
    print(f"\n[INFO] Procesando imagen con Random Forest: {img_path.name}...")
    ...
    print("\n" + "=" * 50)
    print(" RESULTADO CLASIFICACIÓN - RANDOM FOREST ".center(50, "="))
    print("=" * 50)
    ...
    try:
        ...
        plt.imshow(img)
        plt.title(f"Random Forest Predicción: {clase_predicha} ({confianza * 100:.1f}%)", fontsize=14, fontweight='bold')
        plt.axis('off')
        plt.tight_layout()
...
def main():
    if len(sys.argv) > 1:
        ...
        print("=" * 60)
        print(" CLASIFICADOR RANDOM FOREST INTERACTIVO DE PAPAS ".center(60, "="))
        print("=" * 60)
