const API_BASE_URL = 'http://127.0.0.1:8000/predecir_papa?modelo_tipo=mlp';
...
async function clasificarPapa() {
  ...
  const tVal = tempSlider ? tempSlider.value : '2.0';
  const apiPredictUrl = `${API_BASE_URL}&temperatura=${tVal}`;

  try {
    const res = await fetch(apiPredictUrl, {
      method: 'POST',
      body: formData
    });
...
window.clearForm = function() {
  ...
  btnClassify.disabled = true;
  if (tempSlider && tempVal) {
    tempSlider.value = '2.0';
    tempVal.textContent = '2.0';
  }
  ocultarError();
  ocultarResultado();
